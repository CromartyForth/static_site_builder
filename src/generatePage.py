...
def generate_pages_recursive(dir_path_content, template_path, dest_dir_path):
    ...
    generate_pages_recursive_helper(dir_path_content, template_path, dest_dir_path)
    
def generate_pages_recursive_helper(dir_path_content, template_path, dest_dir_path):
    ...
    with os.scandir(dir_path_content) as content:
        for entry in content:
            if entry.is_file() and entry.name.endswith('.md'):
                ...
                generate_page(entry.path, template_path, destination_path)
                continue
            if entry.is_dir():
                destination_path = os.path.join(dest_dir_path, entry.name)
                os.mkdir(destination_path)
                print(f'created {entry.path}')
                generate_pages_recursive_helper(entry.path, template_path, destination_path)
